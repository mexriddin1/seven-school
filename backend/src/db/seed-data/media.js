...
const sourceCandidates = [
  path.resolve(projectRoot, '..', 'seven-school', 'img'),
  path.resolve(projectRoot, '..', 'seven-school', 'images', 'blog'),
];
...
// Lesson extras (mashgulotlar — Darsdan tashqari) — 4 cards
const darslarImgs = [
  { name: 'extra-togaraklar.png', subdir: 'darslar' },
  { name: 'extra-stem.png',       subdir: 'darslar' },
  { name: 'extra-ai.png',         subdir: 'darslar' },
  { name: 'extra-tarbiya.png',    subdir: 'darslar' },
];
...
async function ensureDirs() {
  ...
  await fs.mkdir(path.join(seedTargetDir, 'darslar'), { recursive: true });
}
...
export async function seedMedia() {
  ...
  // Lesson extras images
  for (const { name, subdir } of darslarImgs) {
    const key = name.replace(/\.[^.]+$/, '');
    map[key] = await insertMedia(name, `/uploads/seed/${subdir}/${name}`);
  }
